...
public class UpdatePasswordFrame extends JFrame {
    ...
    private JPasswordField getPf_oldPwd() {
        if (pf_oldPwd == null) {
            ...
            pf_oldPwd.addActionListener(e -> pf_newPwd.requestFocus());
        }
        return pf_oldPwd;
    }
    ...
    private JPasswordField getPf_newPwd() {
        if (pf_newPwd == null) {
            ...
            pf_newPwd.addActionListener(e -> pf_okNewPwd.requestFocus());
        }
        return pf_newPwd;
    }
    ...
    private JPasswordField getPf_okNewPwd() {
        if (pf_okNewPwd == null) {
            ...
            pf_okNewPwd.addActionListener(e -> {
    ...
    private JButton getBtn_update() {
        if (btn_update == null) {
            ...
            btn_update.addActionListener(e -> {
                String oldPwd = new String(pf_oldPwd.getPassword()); // 获得原密码
                String newPwd = new String(pf_newPwd.getPassword()); // 获得新密码
                String okPwd = new String(pf_okNewPwd.getPassword()); // 获得确认密码
                UserDao.updateUser(oldPwd, newPwd, okPwd); // 更新密码
            });
